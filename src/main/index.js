...
function ensureExecutable(filePath) {
  ...
  try {
    ...
    // best-effort; ignore when we cannot adjust permissions
  }
}

function getRendererHtml() {
  const names = [process.env.MAIN_WINDOW_VITE_NAME, "index"].filter(Boolean);
  for (const name of names) {
    const direct = path.join(__dirname, "../renderer", `${name}.html`);
    if (fs.existsSync(direct)) {
      return direct;
    }
    const nested = path.join(__dirname, "../renderer", name, "index.html");
...
function createWindow() {
  ...
  if (devServerUrl) {
    ...
    const htmlPath = getRendererHtml();
    if (!fs.existsSync(htmlPath)) {
      console.warn(
        `Renderer bundle not found at ${htmlPath}. Run "bun run build" or start via "bun run dev" to generate it.`,
      );
    }
...
function resolveCardmakerExecutable() {
  ...
  if (app.isPackaged) {
    candidates.push({
      type: "binary",
      path: path.join(process.resourcesPath, "cardmaker", exeName),
    });
    candidates.push({
      type: "binary",
      path: path.join(process.resourcesPath, "cardmaker", "cardmaker", exeName),
    });
  } else {
    candidates.push({
      type: "binary",
      path: path.join(PROJECT_ROOT, "build", "py", "cardmaker", exeName),
    });
    candidates.push({
      type: "binary",
      path: path.join(
    ...
    candidates.push({ type: "binary", path: path.join(PROJECT_ROOT, exeName) });
  }
  ...
  return { type: "script", path: path.join(PROJECT_ROOT, "cardmaker.py") };
}
...
ipcMain.on("run-generation", (event, args) => {
  ...
  const targetOutputDir = requestedOutput
  ...
  if (typeof testData === "string") {
    ...
    cliArgs.push("--testmode");
    if (trimmedTestData) {
      cliArgs.push(trimmedTestData);
    }
  }
  ...
  if (resolved.type === "binary") {
    ...
    const scriptArgs =
      pythonCmd === "py"
        ? ["-3", "-u", resolved.path, ...cliArgs]
        : ["-u", resolved.path, ...cliArgs];
